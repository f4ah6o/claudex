...
履歴再構成前のscanでは、fork元の`router-for-me/CLIProxyAPI`公開履歴に含まれるGemini、Antigravity、Qwen、iFlow用OAuthクライアント定数を26件検出した。これらはClaudex固有または利用者固有の資格情報ではなく、upstreamの公開履歴に由来することを確認した。公開用履歴は現行treeをroot commitとして再構成し、旧履歴を切り離した。

再構成後のscanでは現行treeに残る公開OAuthクライアント定数を3件検出した。`.gitleaksignore`には、この3件のexact fingerprintだけをbaselineとして登録した。ファイル単位やrule単位の包括的な除外は行わないため、新規・変更されたsecret findingは引き続きCIを失敗させる。
