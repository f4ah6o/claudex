...
初回の全履歴scanでは、fork元の`router-for-me/CLIProxyAPI`公開履歴に含まれるGemini、Antigravity、Qwen、iFlow用OAuthクライアント定数を26件検出した。これらはClaudex固有または利用者固有の資格情報ではなく、同じcommitがupstreamの公開履歴に存在することを確認した。履歴にはこれらの定数を含むcommit identityの重複があるため、baselineには同じ検出に対応する両方のfingerprintを登録した。

`.gitleaksignore`には、この26件に対応するexact fingerprintだけをbaselineとして登録した。ファイル単位やrule単位の包括的な除外は行わないため、新規・変更されたsecret findingは引き続きCIを失敗させる。
