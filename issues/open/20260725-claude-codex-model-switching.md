...
- [ ] Existing Opus and Sonnet compatibility IDs continue to route to the new tier targets.
- [ ] Haiku IDs are not generated by normalization, are absent from the example configuration, and are rejected by the model policy.
- [ ] Direct `gpt-5.6` and `gpt-5.6-*` model IDs remain accepted.
- [ ] Both CLI launchers start with `claude-sonnet-5` and allow interactive `/model` selection among Fable 5, Opus 5, and Sonnet 5.
- [ ] Interactive `/effort max` is sent to Codex as `xhigh`; `low`, `medium`, `high`, and `xhigh` remain distinct; explicit thinking disablement is preserved.
